...
def preprocess_crf_results(crf_file, gold_file):
    '''
    Extract gold and CRF predicted labels from two different files
    
    :param crf_file: TSV with predicted labels from CRF model
    :param gold_file: ConLL file containing gold labels

    :type crf_file: TSV file
    :type gold_file: ConLL file
    '''
    crffile = pd.read_csv(crf_file, delimiter='\t', header=None, encoding= 'utf8', quoting=csv.QUOTE_NONE) 
    goldfile = pd.read_csv(gold_file, delimiter='\t', header=None)
    df = pd.DataFrame(crffile)
    df2 = pd.DataFrame(goldfile) 
    annotations_system = df[1].tolist()
    annotations_gold = df2[3].tolist()
...
def main(argv=None):
    ...
    crf_file = argv[1] #TSV File with CRF results
    gold_file = argv[2] #ConLL file containing gold labels

    annotations_gold, annotations_system = preprocess_crf_results(crf_file, gold_file)
